...
public interface UserRepository extends JpaRepository<User, String> {
	...
	@Query(value=	"SELECT * FROM user WHERE ((first_name LIKE ?1 OR last_name LIKE ?1) AND property_id IS NOT NULL) AND property_id LIKE ?3 UNION "+
					"SELECT * FROM user WHERE ((first_name LIKE ?1 AND last_name LIKE ?2) OR (first_name LIKE ?2 AND last_name LIKE ?1)) AND property_id IS NOT NULL AND property_id LIKE ?3"
					, nativeQuery = true)
