...
public interface AnnouncementRepository extends JpaRepository<Announcement, Integer> {
	...
	@Query(value = "SELECT * FROM announcement a WHERE a.property_id = ?1 AND a.date_to < DATE(?2)", nativeQuery = true )
	public List<Announcement> getAnnByLessDate(int id, String searchDate);
	
	@Query(value = "SELECT * FROM announcement a WHERE a.property_id = ?1 AND a.date_to >= DATE(?2) AND a.date_from <= DATE(?2)", nativeQuery = true )
